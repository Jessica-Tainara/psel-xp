...
const { BuyingAsset, Asset, Account } = require('../models');
...
const buy = async ({ codCliente, codAtivo, qtdeAtivo }, { email }) => {
  ...
  const infos = await prevInfos({
    codCliente,
    codAtivo,
    qtdeAtivo
  }, true);

  if (infos.newAvailableAssets < 0) 
    throw customError(200, `Quantidade de ativos disponível: ${
      infos.availableAssets
    }`);
  
  if (infos.newBalance < 0) 
...
const sell = async (object, {email}) => {
  ...
  if (infos.newPurchasedAssets < 0) 
    throw customError(200, `Quantidade disponível desse ativo para venda: ${
      infos.purchasedAssets
    }`);
...
const getById = async (assetId) => {
  ...
  if (!asset) 
    throw customError(404, "Ativo não encontrado!");
...
const getByClient = async (clientId, {email}) => {
  ...
  if (!purchasedAssets.length) 
    throw customError(404, "Esse cliente não possui ativos");
  

  const result = await Promise.all(purchasedAssets.map(async (item) => {
    const assets = await Asset.findByPk(item.assetId, {
      attributes: [
        ['id', 'codAtivo'],
        ['name', 'nomeAtivo'],
        ['price', 'valor']
    ...
    const result = assets.dataValues || assets;
    return {
      ... result,
      qtdeAtivo: item.qtd
    };
